...
class SocketService {
  ...
  onPostCommented(callback: (data: { postId: string; commentId: string; userId: string; commentCount: number }) => void) {
    this.socket?.on('post:commented', callback)
  }

  onPostSaved(callback: (data: { postId: string; userId: string; saveCount: number }) => void) {
    this.socket?.on('post:saved', callback)
  }

  onPostUnsaved(callback: (data: { postId: string; userId: string; saveCount: number }) => void) {
