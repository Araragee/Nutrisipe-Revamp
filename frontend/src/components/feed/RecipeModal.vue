...
<template>
  <div v-if="show" class="fixed inset-0 z-[100] flex items-center justify-center p-8 bg-black/75 backdrop-blur-md animate-revamp">
    <div
      ...
      <template v-else-if="post">
        ...
        <div class="flex-1 flex flex-col h-full bg-background overflow-hidden relative">
          ...
          <div class="p-8 pb-6">
            ...
            <h2 class="font-montserrat font-extrabold text-3xl tracking-tight leading-tight mb-6">
              {{ post.title }}
            </h2>

            <div class="flex items-center gap-4 py-4 border-y border-glass-border">
              ...
              <button class="px-5 py-2.5 rounded-xl bg-orange-soft text-orange font-bold text-xs hover:bg-orange hover:text-white transition-all">
                Follow
              </button>
            </div>
          </div>
          ...
          <div class="flex-1 overflow-y-auto px-8 pb-8">
            ...
            <div class="grid grid-cols-4 gap-3 mb-8">
              <div v-for="n in nutritionFacts" :key="n.label" class="bg-background-secondary rounded-2xl p-4 text-center border border-glass-border">
            ...
            <div class="flex gap-8 border-b border-glass-border mb-6">
              <button
                ...
                :class="[
                  ...
                  activeTab === t ? 'text-orange border-orange' : 'text-text-muted border-transparent'
                ]"
              >
                {{ t }}
              </button>
            ...
            <div v-if="activeTab === 'ingredients'" class="space-y-3">
               <div v-if="post.recipe?.ingredients && post.recipe.ingredients.length > 0">
                 <div v-for="(ing, idx) in post.recipe.ingredients" :key="idx" class="flex items-center gap-3 p-3.5 bg-background-secondary rounded-xl border border-glass-border">
                   <div class="w-2 h-2 rounded-full bg-orange"></div>
                   <span class="text-sm font-medium">{{ ing.name }}</span>
                   <span class="ml-auto text-xs font-bold text-orange">{{ ing.qty }}</span>
                 </div>
               </div>
               <div v-else class="text-center py-10 text-text-dim text-sm italic">
                  No ingredients listed.
               </div>
            </div>

            <div v-if="activeTab === 'instructions'" class="space-y-6">
               <div v-if="post.recipe?.instructions && post.recipe.instructions.length > 0">
                 <div v-for="step in post.recipe.instructions" :key="step.step" class="flex gap-4">
                   <div class="w-9 h-9 rounded-full bg-orange text-white font-montserrat font-extrabold flex items-center justify-center shrink-0">
                     {{ step.step }}
                   </div>
                   <div>
          ...
          <!-- Footer -->
          <div class="p-6 px-8 border-top border-glass-border bg-background flex gap-4">
            <button class="flex-1 btn-primary py-3.5 !text-sm flex items-center justify-center gap-2">
              <span>🔖</span> Save Recipe
            </button>
            <button class="flex-1 btn-secondary py-3.5 !text-sm flex items-center justify-center gap-2">
              <span>📤</span> Share
            </button>
          </div>
        </div>
