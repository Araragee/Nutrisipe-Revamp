...
watch(isExploreRoute, (val, oldVal) => {
  if (val) {
    if (oldVal === undefined) {
      ...
      setTimeout(() => { navState.value = 'shrunk' }, 250)
    }
  } else {
    if (oldVal === undefined) {
      ...
      navState.value = 'detached'
      setTimeout(() => { navState.value = 'default' }, 350)
    }
  }
}, { immediate: true })
...
<template>
  <div v-if="showShell" class="app-shell flex h-screen overflow-hidden bg-background dark:bg-background">
    ...
    <div class="flex-1 flex flex-col min-w-0 relative">
      <div class="hidden md:block shrink-0 transition-all duration-[350ms] ease-out" :class="navState === 'shrunk' ? 'h-0' : 'h-16'"></div>

      <header 
        :class="[
          'hidden md:flex items-center gap-4 shrink-0 z-50 transition-all duration-[350ms] ease-out',
      ...
      >
        <div 
          class="transition-all duration-[350ms] ease-out overflow-hidden shrink-0 flex items-center"
