...
<template>
  <BaseModal :show="show" :close-button="false" @close="emit('close')">
    <template #header>
      <div class="relative">
        <input
          v-model="searchQuery"
          ...
          autofocus
          class="w-full px-4 py-3 pl-12 pr-4 border border-gray-300 dark:border-zinc-600 rounded-lg bg-white dark:bg-zinc-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all"
        />
        <svg
    ...
    <div class="max-h-96 -mx-6 px-6">
      <div v-if="isSearching" class="py-8 text-center text-gray-500 dark:text-gray-400">
        Searching...
      </div>
      ...
      <div v-else class="divide-y divide-gray-100 dark:divide-zinc-700">
        <div
          v-for="user in searchResults"
          :key="user.id"
          @click="handleUserClick(user.id)"
          class="flex items-center gap-4 p-4 hover:bg-gray-50 dark:hover:bg-zinc-700/50 cursor-pointer transition-colors"
        >
          ...
          <button
            ...
            class="px-4 py-2 text-sm font-medium border border-gray-300 dark:border-zinc-600 rounded-full hover:bg-gray-50 dark:hover:bg-zinc-700 text-gray-700 dark:text-gray-300 transition-all"
          >
            Following
          </button>
          <button
